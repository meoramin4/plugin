...
class ER_DB {
    ...
    public static function create_tables() {
        ...
        // NEW: Table for tracking used discounts
        $discount_table = $wpdb->prefix . 'er_referral_purchases';
        ...
        // === NEW TABLE for referral discount usage ===
        $sql3 = "CREATE TABLE $discount_table (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
            user_id BIGINT UNSIGNED NOT NULL,
            product_id BIGINT UNSIGNED NOT NULL,
        ...
        dbDelta($sql1);
        dbDelta($sql2);
        dbDelta($sql3); // Only one new line!
    }
    ...
    public static function get_statistics_data() {
        global $wpdb;
        $products_table = $wpdb->prefix . 'er_products';
        $claims_table = $wpdb->prefix . 'er_referrals';

        $products = $wpdb->get_results("SELECT id, product_code FROM $products_table ORDER BY id ASC");
        $labels = [];
        $claims = [];

        foreach ($products as $product) {
            $labels[] = $product->product_code ? $product->product_code : $product->id;
            $count = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM $claims_table WHERE product_id = %d", $product->id
            ));
            $claims[] = (int)$count;
        }

        if (empty($labels)) $labels[] = __('No data','email-referral');
        if (empty($claims)) $claims[] = 0;

        return ['labels' => $labels, 'claims' => $claims];
    }

    // === NEW: Helper to check if user already used referral discount for a product ===
    public static function has_used_referral_discount($user_id, $product_id) {
        global $wpdb;
        $discount_table = $wpdb->prefix . 'er_referral_purchases';
        $found = $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM $discount_table WHERE user_id = %d AND product_id = %d", $user_id, $product_id
        ));
        return $found > 0;
    }
}