...
class ER_Ajax {
    ...
    public function change_referral_email() {
        ...
        // === Send referral email on change ===
        // Retrieve referral_token for this referral
        $referral_token = $wpdb->get_var($wpdb->prepare("SELECT referral_token FROM $referral_table WHERE id = %d", $referral_id));
        if ($referral_token && $product_id && $new_email) {
            $registration_page_url = 'https://blueviolet-nightingale-806611.hostingersite.com/affiliate-registration/';
            $registration_link = add_query_arg('token', $referral_token, $registration_page_url);
            $product = get_post($product_id);
            if (class_exists('ER_Email')) {
                ER_Email::send_referral_email($new_email, $registration_link, $product);
            }
        }
